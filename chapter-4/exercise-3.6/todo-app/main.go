...
func todoServerURL() string {
	url := os.Getenv("TODO_SERVER_URL")
	if url == "" {
		url = "http://todo-server-svc:2346"
	}
	return url
}
...
func fetchTodos() string {
	url := todoServerURL() + "/todos"
	log.Printf("fetching todos from todo-server: %s", url)
	resp, err := http.Get(url)
	if err != nil {
		log.Printf("fetch todos failed: %v", err)
		return `<li><span>Todo server unavailable</span></li>`
	}
	defer resp.Body.Close()
	log.Printf("received todos response from todo-server: %s", resp.Status)

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		log.Printf("read todos failed: %v", err)
		return `<li><span>Todo server unavailable</span></li>`
	}
	log.Printf("received %d bytes of todos from todo-server", len(body))
	return string(body)
}
...
func main() {
	...
	http.HandleFunc("/todos", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := r.ParseForm(); err != nil {
			log.Printf("parse form failed: %v", err)
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		todo := strings.TrimSpace(r.PostFormValue("todo"))
		log.Printf("todo submission received: %q (len=%d) from %s", todo, len(todo), r.RemoteAddr)
		if len(todo) > 140 {
			log.Printf("rejected todo over 140 chars: %q len=%d", todo, len(todo))
		}
		log.Printf("forwarding todo to todo-server: %s", todoServerURL()+"/todos")
		if _, err := http.PostForm(todoServerURL()+"/todos", r.PostForm); err != nil {
			log.Printf("forward todo to server failed: %v", err)
		} else {
			log.Printf("todo forwarded to todo-server: %q", todo)
		}
...
func homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, homeTpl, imgHTML, fetchTodos())
}
...
<body style="background:#1b1b2f;color:#e8e8f0;font-family:system-ui,-apple-system,Segoe UI,sans-serif;margin:0;padding:0;min-height:100vh">
	<main style="max-width:560px;margin:0 auto;padding:64px 24px">
		<header style="text-align:center;margin-bottom:32px">
			<p style="margin:0 0 8px;color:#8b8ba8;font-size:13px;letter-spacing:.08em;text-transform:uppercase">Chapter 3 · Exercise 2.2</p>
		...
		<section>
			...
			<form method="post" action="/todos" style="display:flex;gap:12px;margin-bottom:28px">
